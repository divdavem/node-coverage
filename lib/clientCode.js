...
var composeFile = function (fileName, allLines, allConditions, allFunctions, code, options) {
	...
	var clientCode = options.minClientCode ? [
		header, "(", minCreateGlobalObjects.toString(), ")(", uglify.make_string(fileName), ");"
	] :	[
		...
		"(", createGlobalObjects.toString(), ")(",
			...
			uglify.make_string(fileName), ",", JSON.stringify({
				lines : allLines,
				code : options.doHighlight ? highlighted : {},
...
var minCreateGlobalObjects = function (f) {
	var $$_l = this.$$_l;
	if (!this.$$_l) {
		$$_l = function (file, line) {
			var counter = $$_l.runLines[file][line] += 1;
			if (!counter > 0) {
